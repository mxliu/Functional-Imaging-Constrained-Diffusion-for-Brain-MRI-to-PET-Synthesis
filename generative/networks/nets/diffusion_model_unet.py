...
class CrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # print('context before to_k to_q to_v',context.shape)

        key = self.to_k(context)
        value = self.to_v(context)

        # print('context after to_k to_q to_v before Multi-Head Attention',context.shape)
        # print('x',x.shape)
        # print('q',query.shape)
        # print('k',key.shape)
        # print('v',value.shape)
        ...
        # print('context after Multi-Head Attention',context.shape)
        # print('x',x.shape)
        # print('q',query.shape)
        # print('k',key.shape)
        # print('v',value.shape)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        # 1. Self-Attention
        # print('1. Self-Attention',x.shape)
        x = self.attn1(self.norm1(x)) + x

        # print('2. Cross-Attention',x.shape,context.shape)
        # 2. Cross-Attention
        x = self.attn2(self.norm2(x), context=context) + x

        # print('3. Feed-forward',x.shape)
        # 3. Feed-forward
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.spatial_dims == 3:
            ...
            # batch_c, channel_c, height_c, width_c, depth_c = context.shape
        # print('place 1',x.shape,context.shape)
        residual = x
        x = self.norm(x)
        x = self.proj_in(x)

        # print('place 2',x.shape,context.shape)
        inner_dim = x.shape[1]
        # inner_dim_c = context.shape[1]

        if self.spatial_dims == 2:
            x = x.permute(0, 2, 3, 1).reshape(batch, height * width, inner_dim)
        if self.spatial_dims == 3:
            x = x.permute(0, 2, 3, 4, 1).reshape(batch, height * width * depth, inner_dim)
            # context = context.permute(0, 2, 3, 4, 1).reshape(batch, height_c * width_c * depth_c, inner_dim_c)
        
        # print('place 3',x.shape,context.shape)
...
class CrossAttnDownBlock(nn.Module):
    ...
    def forward(
        ...
        for resnet, attn in zip(self.resnets, self.attentions):
            hidden_states = resnet(hidden_states, temb)
            # print('crossattendownblock context',context.shape)
...
class DiffusionModelUNet(nn.Module):
    ...
    def forward(
        ...
        for downsample_block in self.down_blocks:
            # print('4. down context',context.shape)
